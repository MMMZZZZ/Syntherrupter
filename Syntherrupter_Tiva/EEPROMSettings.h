...
private:
    ...
    static constexpr uint32_t BANK_COUNT    = 3;
    const uint32_t BANK_STARTS[3] = {0, 2048, 4096};


    uint32_t byteAddress = 0;
    uint32_t bank = BANK_COUNT; // initialized to value higher than normally possible
    uint32_t tempArray[30]; // must be at least as large as the largest array in use.
    bool EEPROMUpToDate = false;
};
